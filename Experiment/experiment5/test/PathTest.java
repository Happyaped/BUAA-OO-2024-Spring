    ...
    public void testExtractLoopPath() {
       //TODO:[3]
        ...
        if (!fl || !path.isValid()){
            ...
            Path back = path.extractLoopPath();
            assertEquals(false,back == null);
            if (back != null){
                boolean mark1 = false;
                for (int i = 0; i < oldNodes.size()-1; i++) {
                    boolean mark2 = false;
                    for (int j = 0; j < back.getNodes().size(); j++) {
                        if (i+j < oldNodes.size() && Objects.equals(back.getNodes().get(j),oldNodes.get(i+j))){
                ...
                Boolean mark3 = back.getNode(0) == back.getNode(back.getNodes().size() - 1);
                assertEquals(true,mark1 == true && mark3 == true);
            }
        }
