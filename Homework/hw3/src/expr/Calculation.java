...
public class Calculation {
    ...
    public static ArrayList<Standard> deleteDerivation(ArrayList<Standard> list) {
        ArrayList<Standard> back = new ArrayList<>();
        for (Standard item : list) {
            if (item.getComindex().size() == 0) {
                //如果没有exp
                BigInteger temint = item.getCoef().multiply(item.getIndex());
                Standard standard = new Standard(temint,
                        item.getIndex().add(BigInteger.ONE.negate()));
                back = Merge(back,standard);
            } else {
                //如果有exp
                BigInteger temint = item.getCoef().multiply(item.getIndex());
                Standard standard1 = new Standard(temint,
                        item.getIndex().add(BigInteger.ONE.negate()));
                standard1.setComindex(item.getComindex());
                back = Merge(back,standard1);
                ArrayList<Standard> temlist = deleteDerivation(item.getComindex());
                ArrayList<Standard> list1 = new ArrayList<>();
                ArrayList<Standard> list2;
                list1.add(item);
                list2 = Mul(list1, temlist);
                for (Standard item2 : list2) {
                    back = Merge(back,item2);
                }
            }
